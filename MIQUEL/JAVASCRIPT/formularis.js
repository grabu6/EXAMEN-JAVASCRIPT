...
function localitzacions(){
  
    let localitzacions = ["BANYOLES", "OLOT", "GIRONA","BARCELONA", "GRANOLLERS", "LLEIDA","TARRAGONA", "FIGUERES", "MIERES"];
    localitzacions.sort();

    const campSeleccionat= document.getElementsByTagName("select")[1];

    for (let i=0; i<localitzacions.length; i++){
        let option= document.createElement("option");
        option.value= localitzacions[i];
        option.innerHTML= localitzacions[i];
        campSeleccionat.append(option);
    }
}

function provaEdifici(string){
    let edificis=edifici();
    const rExp= /^{edificis()}\+[0-9]{3}\.[a-z]{2}$/;
    return rExp.test(string);
}
function edifici(){

    let paraula= document.getElementsByTagName("select")[1].value;
    let resultatEdifici=paraula.substring(0,5);
    return resultatEdifici;
}    
function validarEdifici(){
    
    const edifici = document.getElementsByClassName("edificis");
    let poblacio = edifici[0].value;

    let imatges=document.getElementsByClassName("imatge");

    if(provaEdifici(poblacio)==true){
        imatges[3].src ="../Imatges/tick.png";
    }else{
        imatges[3].src = "../Imatges/creu.png";
    }
...
window.onload= function start(){
    ...
    let poblacio= document.getElementsByClassName("edificis");

    for(let i =0; i<poblacio.length;i++){
        poblacio[i].addEventListener("change", validarEdifici);
    }
}
